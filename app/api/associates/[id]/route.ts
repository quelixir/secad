...
import { ApiResponse, AssociateInput, AssociateWithRelations } from '@/lib/types'
...
export async function PUT(
  ...
  try {
    const { id } = await params
    const body: Partial<AssociateInput> & { status?: string; resignationDate?: Date } = await request.json()
    ...
    if (body.isIndividual !== undefined) {
      if (body.isIndividual) {
        if (!body.givenNames || !body.familyName) {
          const response: ApiResponse = {
            success: false,
            error: 'Given names and family name are required for individual associates'
          }
    ...
    const associate = await prisma.associate.update({
      ...
      data: {
        ...
        ...(body.previousNames !== undefined && { previousNames: body.previousNames }),
