...
export async function PUT(
  ...
  try {
    ...
    if (body.memberNumber && body.memberNumber !== existingMember.memberNumber) {
    ...
    if (body.memberType) {
      if (body.memberType === 'Individual' && (!body.firstName || !body.lastName)) {
        const response: ApiResponse = {
          success: false,
          error: 'First name and last name are required for individual members'
        }
    ...
    if (body.entityName !== undefined) updateData.entityName = body.entityName || null
    ...
    if (body.country !== undefined) updateData.country = body.country || 'Australia'
    if (body.memberNumber !== undefined) updateData.memberNumber = body.memberNumber || null
...
export async function DELETE(
  ...
  try {
    ...
    // Check if member exists and get holding/transaction counts
    const existingMember = await prisma.member.findUnique({
      where: { id },
      include: {
        _count: {
    ...
    // Check if member has holdings or transactions
    if (existingMember._count.holdings > 0 || 
        existingMember._count.transactionsFrom > 0 || 
        existingMember._count.transactionsTo > 0) {
      const response: ApiResponse = {
        success: false,
        error: 'Cannot delete member with existing holdings or transactions'
      }
      return NextResponse.json(response, { status: 409 })
    }
