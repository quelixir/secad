...
export async function GET(
  ...
  try {
    ...
    const includeSecurities = searchParams.get('include')?.includes('securities')
...
export async function PUT(
  ...
  try {
    ...
    if (body.incorporationDate !== undefined) updateData.incorporationDate = body.incorporationDate || null
...
export async function DELETE(
  ...
  try {
    ...
    if (existingEntity._count.members > 0 || 
        existingEntity._count.securityClasses > 0 || 
        existingEntity._count.transactions > 0) {
      const response: ApiResponse = {
        success: false,
        error: 'Cannot delete entity with existing members, securities, or transactions'
      }
      return NextResponse.json(response, { status: 409 })
    }
