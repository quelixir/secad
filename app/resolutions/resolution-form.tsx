...
import { ResolutionType, ResolutionStatus, ResolutionCategory, type ResolutionInput } from '@/lib/types'
...
export function ResolutionForm({ entityId, entityName, onSaved, resolution }: ResolutionFormProps) {
    const [loading, setLoading] = useState(false)
    const [selectedType, setSelectedType] = useState<string>(resolution?.type || '')
    const [formData, setFormData] = useState<FormData>({
        ...
        resolutionDate: resolution?.resolutionDate ? new Date(resolution.resolutionDate).toISOString().split('T')[0] : '',
        effectiveDate: resolution?.effectiveDate ? new Date(resolution.effectiveDate).toISOString().split('T')[0] : '',
    ...
    useEffect(() => {
        if (selectedType && !resolution) {
            const standardResolution = STANDARD_DIRECTORS_RESOLUTIONS.find(r => r.type === selectedType)
    ...
    const generateResolutionContent = (resolutionInfo: any, entityName: string) => {
...
Note: This resolution may be executed in counterparts and transmitted electronically.`
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = resolution ? `/api/resolutions/${resolution.id}` : '/api/resolutions'
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            {STANDARD_DIRECTORS_RESOLUTIONS.map((res) => (
                                <SelectItem key={res.type} value={res.type}>
                                    <div className="flex flex-col">
                                        <span>{res.title}</span>
                                        <span className="text-xs text-muted-foreground">{res.sections}</span>
                                    </div>
                                </SelectItem>
                            ))}
                        </SelectContent>
                ...
                <div className="space-y-2">
                    ...
                    >
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value={ResolutionStatus.APPROVED}>Approved</SelectItem>
                            <SelectItem value={ResolutionStatus.REJECTED}>Rejected</SelectItem>
                            <SelectItem value={ResolutionStatus.SUPERSEDED}>Superseded</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <button
                        type="button"
                        onClick={() => handleInputChange('resolutionDate', new Date().toISOString().split('T')[0])}
                ...
                <div className="space-y-2">
                    ...
                    <button
                        type="button"
                        onClick={() => handleInputChange('effectiveDate', new Date().toISOString().split('T')[0])}
                        className="text-xs text-blue-600 hover:text-blue-800 underline"
                    >
                        Today
                    </button>
