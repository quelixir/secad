...
export default function MemberViewPage() {
  ...
  const [member, setMember] = useState<
    | (MemberWithRelations & {
        transactionsTo?: any[];
        transactionsFrom?: any[];
      })
    | null
  >(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedSecurityClass, setSelectedSecurityClass] =
    useState<string>("all");
  ...
  const allTransactions = [
    ...(member.transactionsFrom || []).map((t) => ({
      ...t,
      direction: TransactionDirection.OUT as const,
    })),
    ...(member.transactionsTo || []).map((t) => ({
      ...t,
      direction: TransactionDirection.IN as const,
    })),
  ].sort(
    (a, b) =>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {member.memberNumber && (
                        <div className="flex justify-between">
                          ...
                          <span>
                            <div className="flex items-center gap-2">
                              <code className="text-sm bg-muted px-2 rounded">
                                {member.memberNumber}
                              </code>
                              <button
                                onClick={() =>
                      ...
                      <div className="flex justify-between">
                        ...
                        <Badge
                          variant={
                            member.beneficiallyHeld ? "default" : "secondary"
                          }
                        >
                    ...
                    <div className="bg-muted/50 rounded-lg p-4">
                      <div className="text-2xl font-bold">
                        {securitiesSummary
                          ...
                          .toLocaleString(getLocale(), getLocaleOptions())}
                      </div>
                      <div className="text-sm text-muted-foreground">
                        Total Securities
                      </div>
              ...
              <CardContent>
                {allTransactions.length > 0 ? (
                  <>
                    ...
                    <div className="mb-4">
                      <Select
                        value={selectedSecurityClass}
                        onValueChange={setSelectedSecurityClass}
                      >
                        ...
                        <SelectContent>
                          ...
                          {securitiesSummary.map((securityClass) => (
                            <SelectItem
                              key={securityClass.securityClassId}
                              value={securityClass.securityClassId}
                            >
                              {securityClass.securityClassName}{" "}
                              {securityClass.securityClassSymbol &&
                                `(${securityClass.securityClassSymbol})`}
                        ...
                        <TableRow>
                          ...
                          <TableHead>
                            <div className="flex items-center gap-1">
                              ...
                              <span
                                onMouseEnter={(e) =>
                                  showTooltip("Paid Per Security", e)
                                }
                          ...
                          <TableHead>
                            <div className="flex items-center gap-1">
                              ...
                              <span
                                onMouseEnter={(e) =>
                                  showTooltip("Unpaid Per Security", e)
                                }
                                onMouseLeave={hideTooltip}
                              >
                                <HelpCircle className="h-3 w-3 text-muted-foreground cursor-help" />
                          ...
                          <TableRow key={transaction.id}>
                            ...
                            <TableCell>
                              <Button variant="ghost" size="sm" asChild>
                                <Link
                                  href={`/registry/transactions/${transaction.id}`}
                                >
