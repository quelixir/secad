    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await inviteMutation.mutateAsync({
                entityId,
                email: `magic-link+${Math.random().toString(36).slice(2, 10)}@example.org`, // placeholder
                role: inviteRole,
            });
        ...
        <Card>
            ...
            <CardContent>
                ...
                <Table>
                    ...
                    <TableBody>
                        {(collaborators?.data?.userEntityAccess ?? []).map((access: any) => (
                            <TableRow key={access.userId}>
                                <TableCell>{access.user?.name || access.user?.email || access.userId}</TableCell>
                ...
                <ul className="mb-4">
                    {(invitations ?? []).map((inv: any) => (
                        <li key={inv.id} className="flex items-center gap-2 mb-1">
                            <span className="font-mono">/accept-invite?token={inv.token}</span>
                            <span className="text-xs text-muted-foreground">({inv.role})</span>
                            <span className="text-xs">{inv.accepted ? 'Accepted' : 'Pending'}</span>
                        </li>
                    ))}
                </ul>
                <form onSubmit={handleInvite} className="flex gap-2 items-end">
                    ...
                    <Button type="submit" disabled={inviteMutation.status === 'pending'}>Generate Magic Link</Button>
                </form>
                {magicLink && (
