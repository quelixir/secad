...
export function EntityIdentifiers({ identifiers, onChange, disabled = false }: EntityIdentifiersProps) {
    ...
    const onSubmit = (values: IdentifierFormValues) => {
        ...
        const isValid = shouldValidate ? compliancePackRegistration.validateIdentifier(values.country, values.type, values.value) : true
        if (!isValid) {
        ...
        if (isDuplicate) {
            form.setError('type', { message: 'This identifier type already exists for this country' })
            return
        }
    ...
    return (
        <Card>
            <CardHeader>
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Add a new identifier for this entity. Select the country and identifier type.
                                </DialogDescription>
                            </DialogHeader>
                                ...
                                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {packs.map((pack) => (
                                                            <SelectItem key={pack.country} value={pack.country}>
                                                                {pack.country}
                                                            </SelectItem>
                                                        ))}
                                                    </SelectContent>
                                                    ...
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        ...
                                                        <SelectContent>
                                                            {selectedPack.identifierTypes.map((type) => (
                                                                <SelectItem key={type.abbreviation} value={type.abbreviation}>
                                    ...
                                    {form.watch('type') && selectedPack && (
                                        <FormField
                                            ...
                                            name="value"
                                            render={({ field }) => {
                                                const identifierType = selectedPack.identifierTypes.find(t => t.abbreviation === form.watch('type'))
                                                const currentValue = field.value
                                                const country = form.watch('country')
                                                const type = form.watch('type')

                                                // Validate the identifier in real-time
                                                const isValid = currentValue && country && type
                                                    ? compliancePackRegistration.validateIdentifier(country, type, currentValue)
                                                    : null
                                                ...
                                                return (
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <Input
                                                                placeholder={identifierType?.placeholder || 'Enter identifier value'}
                ...
                <CardDescription>
                    Manage legal identifiers for this entity across different jurisdictions
                </CardDescription>
            </CardHeader>
            <CardContent>
                {identifiers.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                        No identifiers added yet. Click &quot;Add Identifier&quot; to get started.
                    </div>
                ) : (
                    <Table>
                        <TableHeader>
